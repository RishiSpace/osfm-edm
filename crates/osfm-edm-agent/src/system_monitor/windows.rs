//! Windows system monitor — user-space event collection without ETW drivers.
//!
//! Lightweight by design: polling-based process snapshots,
//! `ReadDirectoryChangesW`-free recursive scan fallback, and TCP table polling.
//! No kernel driver, no ETW session, no extra dependencies — works from any
//! Windows build of the agent and stays idle-cheap (small sleeps between polls).
...
/// Run the Windows system monitor.
pub async fn run_monitor(config: MonitorConfig, tx: mpsc::Sender<Vec<SystemEvent>>) {
    tracing::info!(
        ...
        "Starting Windows system monitor (user-space polling)"
    );
            ...
            event = event_rx.recv() => {
                match event {
                    ...
                    None => {
                        ...
                        tracing::warn!("All Windows monitor sources exited");
                        break;
                    }
                }
            }
...
/// List processes via sysinfo (cross-platform, no WMI/ETW dependency).
...
/// Emits FileAccessed{Write} for new or changed files, {Delete} for removed.
...
/// Network watcher: `netstat -ano -p TCP` snapshot, new-connection events.
...
fn tcp_connections() -> std::collections::HashMap<String, u32> {
    ...
    for line in text.lines() {
        ...
        out.insert(format!("{}->{}", parts[1], parts[2]), pid);
    }
    out
}
